...
This lesson will pick up from the `starter-code/catcollector` folder in this repo:

1. Clone down this repo (do not fork). 
1. Change directory into the repo, then `cd starter-code/catcollector`. 

Then you can start the web server with `python3 manage.py runserver`. 

If at any point you have trouble finding a bug or get behind, you can sync your code with mine: 
...
1. How does our app know which html to send back to the browser? 
2. What functionality does our app currently have? 
...
As usual, Lab time is to be spent implementing the same features in your Spotify Django project.
